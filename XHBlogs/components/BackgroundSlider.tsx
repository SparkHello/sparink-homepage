...
export default function BackgroundSlider() {
  ...
  const displayImages = isDesktop ? images : [siteConfig.mobileBgImage];

  useEffect(() => {
    const mediaQuery = window.matchMedia('(min-width: 768px)');
    const syncViewport = () => {
      setIsDesktop(mediaQuery.matches);
      if (!mediaQuery.matches) setIndex(0);
    };
      ...
      {displayImages.map((img, i) => (
        <div
          ...
          style={{
            ...
            opacity: i === index ? 1 : 0,
            // 解决层级重叠导致的渲染压力
            visibility: Math.abs(i - index) <= 1 || (i === displayImages.length - 1 && index === 0) ? 'visible' : 'hidden'
          }}
        />
      ))}
    </div>
